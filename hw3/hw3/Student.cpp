...
Student::Student(const std::string& fullName, const std::string& gender, int age,
                 const std::string& group, const std::string& college,
                 const std::string& homePets, const std::string& sport)
    : Person(fullName, gender, age), group_(group), college_(college),
      homePets_(homePets), sport_(sport), averageGrade_(0.0) {}
...
// ДЗ3 (Варіант А): прямий доступ до protected isAdult() та age_.
