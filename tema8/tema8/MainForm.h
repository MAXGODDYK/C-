...
// Тема 8 — об'єднано СР8 + ДЗ8. Повний файловий цикл поверх DataGridView + Settings.
// СР8 (основа Save/Load, у SaveToFile/LoadFromFile):
//   #1 маркер типу S/T/F перед полями; #2 потоки закрито у finally;
//   #3 збереження Settings (LastPath); #4 очищення vector (із delete) перед load.
// ДЗ8 (надбудова):
...
namespace Portfolio {
    ...
    private:
        ...
        void InitializeComponent() {
            ...
            this->Text = L"StudentPortfolio - Theme 8 (combined SR8 + HW8)";
        ...
        int LoadFromFile(String^ path) {
            ...
            finally { if (sr != nullptr) { sr->Close(); } }   // СР8 #2: читач закрито
        }
