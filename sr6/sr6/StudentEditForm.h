...
// ТЕМА 6. Модальне вікно для редагування студента.
// Вхід  -> через параметризований конструктор.
// Вихід -> через властивості тільки для читання (EditedAge тощо).
// Зв'язок із MainForm лише через ShowDialog() + DialogResult,
// БЕЗ прямого доступу до елементів вікна ззовні.
...
namespace Portfolio {
    ...
    public:
        // Конструктор отримує поточні дані студента і заповнює поля.
    ...
    private:
        ...
        // Перевірити -> зберегти результат -> DialogResult::OK -> закрити.
    ...
    public:
        // Властивості тільки для читання, що повертають відредаговані дані.
