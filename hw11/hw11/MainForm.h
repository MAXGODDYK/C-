...
// ДЗ11 (Тема 11). Повний цикл експорту в Excel через COM Interop.
//   #1 Повний цикл: створення книги -> заповнення -> форматування -> збереження.
//   #2 ProgressBar, що оновлюється під час експорту (Application::DoEvents()).
//   #3 Окремі аркуші за типом: Students / Teachers / Staff.
//   #4 Перевірка наявності Excel на ПК перед запуском.
//   #5 Варіант А: стовпчикова діаграма (Chart) за віком студентів.
//   + звільнення COM-об'єктів у finally (Marshal::ReleaseComObject + GC).
...
namespace Portfolio {
    ...
    private:
        ...
        void InitializeComponent() {
            ...
            this->Text = L"StudentPortfolio - Theme 11 (HW11: full Excel export)";
        ...
        int FillSheet(Excel::Worksheet^ ws, std::vector<Person*>& group, int& progressValue) {
            ...
            int rowIndex = 1;                 // 1-based; дані з рядка 2
            for (Person* p : group) {
                rowIndex++;
            ...
            // Автоформат: Bold-заголовок, межі, автоширина.
        ...
        System::Void btnExportExcel_Click(System::Object^ sender, EventArgs^ e) {
            ...
            try {
                ...
                // Звільнення COM-об'єктів (без завислого EXCEL.EXE).
