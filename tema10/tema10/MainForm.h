...
// Тема 10 — об'єднано СР10 + ДЗ10. Друк звітів та експорт даних.
// СР10 (маркери // СР10 #k у методах друку/CSV):
//   #1 printStartIndex_ — поле форми, скид лише в BeginPrint;
//   #2 перевірка межі сторінки (y+lineHeight>bottom) -> HasMorePages=true;
//   #3 заголовок поза циклом даних; #4 CSV-екранування + UTF8.
// ДЗ10 (маркери // ДЗ10 #k):
...
namespace Portfolio {
    ...
    private:
        ...
        void InitializeComponent() {
            ...
            this->Text = L"StudentPortfolio - Theme 10 (combined SR10 + HW10)";
        ...
        System::Void printDocument1_BeginPrint(System::Object^ sender, PrintEventArgs^ e) {
            printStartIndex_ = 0;   // СР10 #1: скид індексу друку лише тут
        ...
        System::Void printDocument1_PrintPage(System::Object^ sender, PrintPageEventArgs^ e) {
            ...
            // СР10 #3: заголовок — поза циклом даних, на початку сторінки.
            ...
            while (printStartIndex_ < static_cast<int>(people_->size())) {
                // СР10 #2: перевірка нижньої межі сторінки -> нова сторінка.
        ...
        // ===================== CSV =====================
        // СР10 #4: екранування CSV — обгортання в лапки + подвоєння внутрішніх.
