...
Staff::Staff(const std::string& firstName, const std::string& midName, const std::string& lastName,
             const std::string& gender, int age, const std::string& position,
             const std::string& department, const std::string& hireDate)
    : Person(firstName, midName, lastName, gender, age), position_(position), department_(department), hireDate_(hireDate) {}
...
std::string Staff::getFormalGreeting() const { return "Colleague " + getFullName() + " from " + department_ + "!"; }
...
Staff::~Staff() { std::cout << "[~Staff] " << getLastName() << " (staff) deleted." << std::endl; }
